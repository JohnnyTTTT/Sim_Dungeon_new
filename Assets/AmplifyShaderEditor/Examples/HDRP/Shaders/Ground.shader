...
Shader "ASESampleShaders/Decals Muddy Ground/Ground"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 uv_DirtHeight = inputMesh.uv0.xy * _DirtHeight_ST.xy + _DirtHeight_ST.zw;
				...
				#endif
				float3 vertexValue = ( ( SAMPLE_TEXTURE2D_LOD( _DirtHeight, sampler_DirtHeight, uv_DirtHeight, 0.0 ) * float4( inputMesh.normalOS , 0.0 ) ) * _Height ).rgb;
						 ...
						 )
			{
				...
				float2 uv_DirtAlbedo = packedInput.ase_texcoord5.xy * _DirtAlbedo_ST.xy + _DirtAlbedo_ST.zw;
				float4 tex2DNode11 = SAMPLE_TEXTURE2D( _DirtAlbedo, sampler_DirtAlbedo, uv_DirtAlbedo );
				
				float2 uv_DirtNormal = packedInput.ase_texcoord5.xy * _DirtNormal_ST.xy + _DirtNormal_ST.zw;
				

				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;

				surfaceDescription.BaseColor = tex2DNode11.rgb;
				surfaceDescription.Normal = UnpackNormalScale( SAMPLE_TEXTURE2D( _DirtNormal, sampler_DirtNormal, uv_DirtNormal ), 1.0f );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_DirtAlbedo = packedInput.ase_texcoord2.xy * _DirtAlbedo_ST.xy + _DirtAlbedo_ST.zw;
				float4 tex2DNode11 = SAMPLE_TEXTURE2D( _DirtAlbedo, sampler_DirtAlbedo, uv_DirtAlbedo );
				
				float2 uv_DirtNormal = packedInput.ase_texcoord2.xy * _DirtNormal_ST.xy + _DirtNormal_ST.zw;
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 uv_DirtHeight = inputMesh.ase_texcoord.xy * _DirtHeight_ST.xy + _DirtHeight_ST.zw;
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_DirtNormal = packedInput.ase_texcoord3.xy * _DirtNormal_ST.xy + _DirtNormal_ST.zw;
				
				float2 uv_DirtAlbedo = packedInput.ase_texcoord3.xy * _DirtAlbedo_ST.xy + _DirtAlbedo_ST.zw;
				float4 tex2DNode11 = SAMPLE_TEXTURE2D( _DirtAlbedo, sampler_DirtAlbedo, uv_DirtAlbedo );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 uv_DirtAlbedo = packedInput.ase_texcoord7.xy * _DirtAlbedo_ST.xy + _DirtAlbedo_ST.zw;
				float4 tex2DNode11 = SAMPLE_TEXTURE2D( _DirtAlbedo, sampler_DirtAlbedo, uv_DirtAlbedo );
				
				float2 uv_DirtNormal = packedInput.ase_texcoord7.xy * _DirtNormal_ST.xy + _DirtNormal_ST.zw;
...
ASEEND*/
//CHKSM=C76F8870E92FE3624946DF015672E7875CE35F95