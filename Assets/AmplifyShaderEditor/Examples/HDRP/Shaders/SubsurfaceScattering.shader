...
Shader "ASESampleShaders/SRP HD Material Types/Subsurface Scattering"
{
	Properties
	{
		...
		[DiffusionProfile]_Profile("Profile", Float) = 0[HideInInspector]_Profile_Asset("Profile", Vector) = ( 0, 0, 0, 0 )
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				surfaceDescription.BaseColor = tex2D( _BaseMap, uv_BaseMap ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_Thickness = packedInput.ase_texcoord2.xy * _Thickness_ST.xy + _Thickness_ST.zw;
				
				float2 uv_SubsurfaceMask = packedInput.ase_texcoord2.xy * _SubsurfaceMask_ST.xy + _SubsurfaceMask_ST.zw;
