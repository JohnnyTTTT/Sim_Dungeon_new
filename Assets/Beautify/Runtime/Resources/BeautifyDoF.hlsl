...
#define BEAUTIFY_DOF_FX
	...
	// Copyright 2020-2024 Kronnect - All Rights Reserved.
    ...
    float4 FragCoC (Varyings input) : SV_Target {
        ...
        float2 scaledUV = input.uv * _InputScale;
        float4 pixel  = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, scaledUV);
               pixel.rgb /= _HDROutputLevel;
        pixel         = clamp(pixel, 0.0.xxxx, _BokehData3.xxxx);
        float coc = getCoc(input.uv) / COC_BASE;
    ...
    float4 FragCoCDebug (Varyings input) : SV_Target {
        ...
        float4 pixel  = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, scaledUV);
        pixel.rgb /= _HDROutputLevel;
        float  CoC   = min(getCoc(input.uv) / 16.0, 1.0);
    ...
    float4 FragBlur (Varyings input): SV_Target {
        ...
        float4 sum     = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, uv);
        ...
        for (int k=1;k<16;k++) {
            if (k<sampleCount) {
                ...
                float4 pixel1      = SAMPLE_TEXTURE2D_X_LOD(_InputTex, sampler_Beautify_LinearClamp, disp1.xy, 0);
                ...
                float4 pixel2 = SAMPLE_TEXTURE2D_X_LOD(_InputTex, sampler_Beautify_LinearClamp, disp2.xy, 0);
   ...
   float4 FragBlurCoC (VaryingsDoFCross input): SV_Target {
        ...
        float coc1    = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, uv1).a;
        float coc2    = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, uv2).a;
        float coc3    = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, uv3).a;
        float coc4    = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, uv4 ).a;

        float w0      = 0.2270270270;

        float4 pixel = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, input.uv);
    ...
    float4 FragCopyBokeh(Varyings input) : SV_Target
    {
        ...
        float4 bokeh = SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_PointClamp, input.uv);
        return bokeh;
    }
