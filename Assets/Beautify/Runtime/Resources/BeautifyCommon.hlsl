...
SAMPLER(sampler_Beautify_PointClamp);
SAMPLER(sampler_Beautify_LinearClamp);
SAMPLER(sampler_Beautify_LinearRepeat);
...
// Common functions
    ...
    float4 FragCopyExact(Varyings input) : SV_Target
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        return SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_PointClamp, input.uv);
    }
    ...
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        return SAMPLE_TEXTURE2D_X(_InputTex, sampler_Beautify_LinearClamp, input.uv);
    }
    ...
    float2 internal_ClampAndScaleUV(float2 UV, float2 texelSize, float numberOfTexels, float2 scale) {
        float2 maxCoord = 1.0f - numberOfTexels * texelSize;
        return min(UV, maxCoord) * scale;
    }

    float2 clampUV(float2 uv) {
        return internal_ClampAndScaleUV(uv, _SourceTex_TexelSize.xy, 1.0, _InputScale.xy);
    }
