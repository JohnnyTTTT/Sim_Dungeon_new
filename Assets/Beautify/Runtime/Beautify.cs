...
namespace BeautifyHDRP {
    ...
    public sealed class Beautify : CustomPostProcessVolumeComponent, IPostProcessComponent {
        ...
        public ClampedFloatParameter tonemapAGXGamma = new ClampedFloatParameter(2.5f, 0, 5f);
        ...
        public ClampedFloatParameter depthOfFieldBokehThreshold = new ClampedFloatParameter(1f, 0.001f, 3f);
        ...
        [Header("Film Grain & Artifacts")]
        public BoolParameter filmGrain = new BoolParameter(false);
        [Tooltip("Amount of noise on screen.")]
        public ClampedFloatParameter filmGrainIntensity = new ClampedFloatParameter(0f, 0, 1f);
        // Add Film Grain Resolution parameter
        [Tooltip("Controls the resolution of the film grain pattern. Lower values = coarser grain, higher = finer grain.")]
        public ClampedFloatParameter filmGrainResolution = new ClampedFloatParameter(0.85f, 0.001f, 1f);
        [Tooltip("Controls how much grain is visible on bright areas. 0 for equally noise on all areas, 1 for reduced noise on bright areas.")]
        public ClampedFloatParameter filmGrainLumaAttenuation = new ClampedFloatParameter(0.8f, 0, 1f);
        [Tooltip("Controls the frequency of dirt spots appearing on the film.")]
        public ClampedFloatParameter filmGrainDirtSpotsAmount = new ClampedFloatParameter(0.0f, 0, 1f);
        [Tooltip("Controls the visibility/brightness of dirt spots.")]
        public ClampedFloatParameter filmGrainDirtSpotsIntensity = new ClampedFloatParameter(0.2f, 0, 1f);
        [Tooltip("Controls the frequency of scratches appearing on the film.")]
        public ClampedFloatParameter filmGrainScratchesAmount = new ClampedFloatParameter(0.0f, 0, 1f);
        [Tooltip("Controls the visibility/brightness of scratches.")]
        ...
        static class ShaderParams {
            ...
            public static int tonemapGamma = Shader.PropertyToID("_TonemapAGXGamma");
            ...
            public static int hdrOutputLevel = Shader.PropertyToID("_HDROutputLevel");
        ...
        public override void Render (CommandBuffer cmd, HDCamera camera, RTHandle source, RTHandle destination) {
            ...
            m_Material.SetFloat(ShaderParams.intensity, intensity.value);
            bool isHDROutput = HDROutputSettings.main.active;

            m_Material.SetFloat(ShaderParams.hdrOutputLevel, isHDROutput ? 1000 : 1);
            ...
            float tempSharpen = Mathf.Clamp(sharpen.value - currSens, 0, sharpen.value);
            float tempRelaxation = sharpenRelaxation.value;
            if (isHDROutput) {
                tempSharpen *= 0.001f;
                tempRelaxation *= 1000f;
            }

            Vector4 sharpenData = new Vector4(tempSharpen, sharpenDepthThreshold.value, sharpenClamp.value, tempRelaxation);
            m_Material.SetVector(ShaderParams.sharpen, sharpenData);
            float dither = pixelateSize.value <= 1 ? ditherStrength.value : 0;
            ...
            m_Material.SetVector(ShaderParams.colorBoost, new Vector4(brightness.value, cont, saturate.value, daltonize.value * 10f));
            ...
            m_Material.DisableKeyword(ShaderParams.SKW_TONEMAP_ACES_FITTED);
            m_Material.DisableKeyword(ShaderParams.SKW_TONEMAP_AGX);
            if (tonemap.value == TonemapOperator.ACES) {
                m_Material.SetFloat(ShaderParams.tonemapExposure, tonemapExposure.value);
                m_Material.EnableKeyword(ShaderParams.SKW_TONEMAP_ACES_FITTED);
            }
            else if (tonemap.value == TonemapOperator.AGX) {
                m_Material.SetFloat(ShaderParams.tonemapExposure, tonemapExposure.value);
                m_Material.SetFloat(ShaderParams.tonemapGamma, tonemapAGXGamma.value);
                m_Material.EnableKeyword(ShaderParams.SKW_TONEMAP_AGX);
            }
        ...
        void DoFilmGrain (HDCamera cam) {
            if (filmGrain.value) {
                // Make film grain resolution independent by scaling inversely with screen resolution
                float resolutionScale = 1920f / cam.camera.pixelWidth; // Use 1920 as reference resolution
                float adjustedResolution = filmGrainResolution.value * resolutionScale;
                m_Material.SetVector(ShaderParams.filmGrainData, new Vector4(filmGrainIntensity.value, filmGrainLumaAttenuation.value, adjustedResolution, 0));
                float dirtSpotsIntensity = filmGrainDirtSpotsIntensity.value;
                float filmScrachesAmount = 1f - filmGrainScratchesAmount.value * 0.05f;
                float filmScrachesIntensity = filmGrainScratchesIntensity.value;
                m_Material.SetVector(ShaderParams.filmArtifactsData, new Vector4(dirtSpotsIntensity > 0 ? 1f - filmGrainDirtSpotsAmount.value * 0.1f : 0, dirtSpotsIntensity, filmScrachesIntensity > 0 ? filmScrachesAmount : 0, filmScrachesIntensity));
                m_Material.EnableKeyword(ShaderParams.SKW_FILM_GRAIN);
            }
            else {
                m_Material.DisableKeyword(ShaderParams.SKW_FILM_GRAIN);
            }
        }
