...
Shader "AmplifyShaderPack/Two Sided With Face"
{
	Properties
	{
		...
		[HideInInspector] _Cutoff( "Cutoff", Float ) = 0.45
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord14 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float4 switchResult2 = (((IsFrontFace>0)?(( tex2D( _FrontAlbedo, texCoord14 ) * _FrontColor )):(( tex2D( _BackAlbedo, texCoord14 ) * _BackColor ))));
				
				float3 switchResult3 = (((IsFrontFace>0)?(UnpackNormalScale( tex2D( _FrontNormalMap, texCoord14 ), 1.0f )):(UnpackNormalScale( tex2D( _BackNormalMap, texCoord14 ), 1.0f ))));
				...
				surfaceDescription.BaseColor = switchResult2.rgb;
				surfaceDescription.Normal = switchResult3;
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord14 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float3 switchResult3 = (((IsFrontFace>0)?(UnpackNormalScale( tex2D( _FrontNormalMap, texCoord14 ), 1.0f )):(UnpackNormalScale( tex2D( _BackNormalMap, texCoord14 ), 1.0f ))));
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 texCoord14 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float4 switchResult2 = (((IsFrontFace>0)?(( tex2D( _FrontAlbedo, texCoord14 ) * _FrontColor )):(( tex2D( _BackAlbedo, texCoord14 ) * _BackColor ))));
...
WireConnection;15;10;27;0
ASEEND*/
//CHKSM=DD541074ECF70D0788D53D779547E6D2847E36E3