	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_Normals = packedInput.ase_texcoord5.xyz.xy * _Normals_ST.xy + _Normals_ST.zw;
				float3 tex2DNode10 = UnpackNormalScale( tex2D( _Normals, uv_Normals ), 1.0f );
				
				float2 uv_Metallic = packedInput.ase_texcoord5.xyz.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				...
				surfaceDescription.BaseColor = ( tex2D( _Albedo, uv_Albedo ) * 0.5 ).rgb;
				...
				surfaceDescription.Emission = ( ( 1.0 - 0.5 ) * texCUBE( _Cubemap, worldRefl9 ) ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_Normals = packedInput.ase_texcoord2.xyz.xy * _Normals_ST.xy + _Normals_ST.zw;
				float3 tex2DNode10 = UnpackNormalScale( tex2D( _Normals, uv_Normals ), 1.0f );
				
				float2 uv_Metallic = packedInput.ase_texcoord2.xyz.xy * _Metallic_ST.xy + _Metallic_ST.zw;
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 uv_Normals = packedInput.ase_texcoord7.xyz.xy * _Normals_ST.xy + _Normals_ST.zw;
				float3 tex2DNode10 = UnpackNormalScale( tex2D( _Normals, uv_Normals ), 1.0f );
				
				float2 uv_Metallic = packedInput.ase_texcoord7.xyz.xy * _Metallic_ST.xy + _Metallic_ST.zw;
...
WireConnection;14;7;25;0
WireConnection;14;6;7;0
ASEEND*/
//CHKSM=9DE00151A0E6D08504FD7BFD13AABB8FB18DE697