	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction( AttributesMesh inputMesh  )
			{
				...
				float2 texCoord2_g6 = inputMesh.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float2 vertexToFrag10_g6 = ( ( texCoord2_g6 * (unity_LightmapST).xy ) + (unity_LightmapST).zw );
				o.ase_texcoord1.xy = vertexToFrag10_g6;
					...
					)
			{
				...
				float2 vertexToFrag10_g6 = packedInput.ase_texcoord1.xy;
				...
				float4 staticSwitch15_g6 = float4( 34.49324, 2.2, 0, 0 );
				#else
				float4 staticSwitch15_g6 = float4( 2, 2.2, 0, 0 );
				#endif
				#ifdef UNITY_LIGHTMAP_FULL_HDR
				float4 staticSwitch13_g6 = float4( 0, 0, 0, 0 );
				#else
				float4 staticSwitch13_g6 = staticSwitch15_g6;
				#endif
				float3 decodeLightMap6_g6 = DecodeLightmap(localSampleLightmapHD11_g6,staticSwitch13_g6);
				float2 uv_MainTex = packedInput.ase_texcoord1.zw * _MainTex_ST.xy + _MainTex_ST.zw;
				float4 appendResult8 = (float4(( ( decodeLightMap6_g6 * GetCurrentExposureMultiplier() ) * (tex2D( _MainTex, uv_MainTex )).rgb ) , 1.0));
				

				SurfaceDescription surfaceDescription = (SurfaceDescription)0;

				surfaceDescription.Color = appendResult8.xyz;
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction( AttributesMesh inputMesh  )
			{
				...
				float2 texCoord2_g6 = inputMesh.uv1.xy * float2( 1,1 ) + float2( 0,0 );
				float2 vertexToFrag10_g6 = ( ( texCoord2_g6 * (unity_LightmapST).xy ) + (unity_LightmapST).zw );
				o.ase_texcoord2.xy = vertexToFrag10_g6;
			...
			float4 Frag( PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 vertexToFrag10_g6 = packedInput.ase_texcoord2.xy;
				float2 UV11_g6 = vertexToFrag10_g6;
				float4 localSampleLightmapHD11_g6 = SampleLightmapHD11_g6( UV11_g6 );
				#ifdef UNITY_LIGHTMAP_RGBM_ENCODING
				float4 staticSwitch15_g6 = float4( 34.49324, 2.2, 0, 0 );
				...
				#endif
				float3 decodeLightMap6_g6 = DecodeLightmap(localSampleLightmapHD11_g6,staticSwitch13_g6);
				float2 uv_MainTex = packedInput.ase_texcoord2.zw * _MainTex_ST.xy + _MainTex_ST.zw;
				float4 appendResult8 = (float4(( ( decodeLightMap6_g6 * GetCurrentExposureMultiplier() ) * (tex2D( _MainTex, uv_MainTex )).rgb ) , 1.0));
...
ASEEND*/
//CHKSM=4418459EC774174A4EFAA7C81F6B343B0C4F7A77