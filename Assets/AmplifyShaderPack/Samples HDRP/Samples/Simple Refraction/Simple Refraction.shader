	...
	SubShader
	{
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_RefractionMap = packedInput.ase_texcoord1.xy * _RefractionMap_ST.xy + _RefractionMap_ST.zw;
				float4 fetchOpaqueVal8 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xy + (( UnpackNormalScale( tex2D( _RefractionMap, uv_RefractionMap ), 1.0f ) * _Distortion )).xy ).xy, 0.0, 1.0);
				

				SurfaceDescription surfaceDescription = (SurfaceDescription)0;

				surfaceDescription.Color = fetchOpaqueVal8.rgb;
		...
		Pass
		{
			...
			float4 Frag( PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 ase_grabScreenPos = ASE_ComputeGrabScreenPos( screenPos );
				float4 ase_grabScreenPosNorm = ase_grabScreenPos / ase_grabScreenPos.w;
				float2 uv_RefractionMap = packedInput.ase_texcoord3.xy * _RefractionMap_ST.xy + _RefractionMap_ST.zw;
				float4 fetchOpaqueVal8 = ASEHDSampleSceneColor(( (ase_grabScreenPosNorm).xy + (( UnpackNormalScale( tex2D( _RefractionMap, uv_RefractionMap ), 1.0f ) * _Distortion )).xy ).xy, 0.0, 1.0);
...
ASEEND*/
//CHKSM=54188F1A0A59FFC6F2C0ABDA262EA31C36424DCE