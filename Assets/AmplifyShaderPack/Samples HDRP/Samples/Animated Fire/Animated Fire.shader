	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_BurnMask = packedInput.ase_texcoord5.xy * _BurnMask_ST.xy + _BurnMask_ST.zw;
				float2 panner2_g3 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + texCoord4);
				float3 hdEmission23 = ASEGetEmissionHDRColor(( ( tex2D( _BurnMask, uv_BurnMask ) * tex2D( _FireTexture, panner2_g3 ) ) * ( _FireIntensity * ( _TimeParameters.y + 1.5 ) ) ).rgb,_EmissionIntensity,_EmissionWeight,GetInverseCurrentExposureMultiplier());
				

				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;

				surfaceDescription.BaseColor = tex2D( _Albedo, texCoord4 ).rgb;
				surfaceDescription.Normal = UnpackNormalScale( tex2D( _Normals, texCoord4 ), 1.0f );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = 0;

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = tex2D( _Specular, texCoord4 ).rgb;
				#endif

				surfaceDescription.Smoothness = _Smoothness;
				surfaceDescription.Occlusion = 1;
				surfaceDescription.Emission = hdEmission23;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_BurnMask = packedInput.ase_texcoord2.xy * _BurnMask_ST.xy + _BurnMask_ST.zw;
				float2 panner2_g3 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + texCoord4);
				float3 hdEmission23 = ASEGetEmissionHDRColor(( ( tex2D( _BurnMask, uv_BurnMask ) * tex2D( _FireTexture, panner2_g3 ) ) * ( _FireIntensity * ( _TimeParameters.y + 1.5 ) ) ).rgb,_EmissionIntensity,_EmissionWeight,GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
					 ...
					 )
			{
				...
				float2 uv_BurnMask = packedInput.ase_texcoord7.xy * _BurnMask_ST.xy + _BurnMask_ST.zw;
				float2 panner2_g3 = ( ( _TimeParameters.x * 0.05 ) * float2( -1,0 ) + texCoord4);
				float3 hdEmission23 = ASEGetEmissionHDRColor(( ( tex2D( _BurnMask, uv_BurnMask ) * tex2D( _FireTexture, panner2_g3 ) ) * ( _FireIntensity * ( _TimeParameters.y + 1.5 ) ) ).rgb,_EmissionIntensity,_EmissionWeight,GetInverseCurrentExposureMultiplier());
...
WireConnection;26;5;16;0
WireConnection;26;7;15;0
WireConnection;26;6;23;0
ASEEND*/
//CHKSM=F5205539230C75C087E4CD30D7A1A057B010CF9F