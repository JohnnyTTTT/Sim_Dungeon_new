...
Shader "AmplifyShaderPack/Material Sample Billboard Spherical"
{
	...
	SubShader
	{
		...
		Tags { "RenderPipeline"="HDRenderPipeline" "RenderType"="TreeBillboard" "Queue"="Geometry" }
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				inputMesh.tangentOS.xyz = normalize( mul( float4( inputMesh.tangentOS.xyz , 0 ), rotationCamMatrix )).xyz;
				//This unfortunately must be made to take non-uniform scaling into account;
				//Transform to world coords, apply rotation and transform back to local;
				inputMesh.positionOS = mul( float4(inputMesh.positionOS,0) , GetObjectToWorldMatrix() ).xyz;
				inputMesh.positionOS = mul( float4(inputMesh.positionOS,0) , rotationCamMatrix ).xyz;
				inputMesh.positionOS = mul( float4(inputMesh.positionOS,0) , GetWorldToObjectMatrix() ).xyz;
				...
				float dotResult960_g38 = dot( normalizedWorldNormal , ase_viewDirSafeWS );
				float3 lerpResult966_g38 = lerp( inputMesh.normalOS , -inputMesh.normalOS , step( dotResult960_g38 , -1.0 ));
				
				float4 appendResult956_g38 = (float4(cross( inputMesh.normalOS , float3( 0, 0, 1 ) ) , -1.0));
				...
				inputMesh.normalOS = lerpResult966_g38;
				inputMesh.tangentOS = appendResult956_g38;
