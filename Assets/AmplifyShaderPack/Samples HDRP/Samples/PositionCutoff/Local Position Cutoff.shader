	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float temp_output_15_0 = saturate( ( ( (( _UseObjectSpace )?( packedInput.ase_texcoord6.xyz.y ):( PositionWS.y )) + _StartPoint ) / _Distribution ) );
				...
				surfaceDescription.Normal = UnpackNormalScale( tex2D( _Normals, uv_Normals ), 1.0f );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = temp_output_49_0.r;

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = 0;
				#endif

				surfaceDescription.Smoothness = ( temp_output_49_0 * _SmoothnessFactor ).r;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float temp_output_15_0 = saturate( ( ( (( _UseObjectSpace )?( packedInput.ase_texcoord4.xyz.y ):( ase_positionWS.y )) + _StartPoint ) / _Distribution ) );
				float clampResult30 = clamp( temp_output_15_0 , _UnderwaterInfluence , 1.0 );
				float4 lerpResult52 = lerp( _Tint , tex2D( _Albedo, uv_Albedo ) , clampResult30);
		...
		Pass
		{
						 ...
						 )
			{
				...
				float temp_output_15_0 = saturate( ( ( (( _UseObjectSpace )?( packedInput.ase_texcoord4.xyz.y ):( PositionWS.y )) + _StartPoint ) / _Distribution ) );
				float4 temp_output_49_0 = ( tex2D( _Metallic, uv_Metallic ) + ( 1.0 - temp_output_15_0 ) );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float temp_output_15_0 = saturate( ( ( (( _UseObjectSpace )?( packedInput.ase_texcoord8.xyz.y ):( PositionWS.y )) + _StartPoint ) / _Distribution ) );
				float clampResult30 = clamp( temp_output_15_0 , _UnderwaterInfluence , 1.0 );
				float4 lerpResult52 = lerp( _Tint , tex2D( _Albedo, uv_Albedo ) , clampResult30);
...
WireConnection;61;4;49;0
WireConnection;61;7;51;0
WireConnection;61;8;20;1
ASEEND*/
//CHKSM=A735D03755D034BAF73F4D9CA2B6E4792F74BB08