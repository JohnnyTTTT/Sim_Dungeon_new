	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				surfaceDescription.BaseColor = ( tex2D( _Albedo, uv_Albedo ) * tex2D( _ScreenDetail, ( (ScreenPosNorm).xy * float2( 8,6 ) ) ) ).rgb;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				surfaceDescription.BaseColor = ( tex2D( _Albedo, uv_Albedo ) * tex2D( _ScreenDetail, ( (ase_positionSSNorm).xy * float2( 8,6 ) ) ) ).rgb;
				surfaceDescription.Normal = float3( 0, 0, 1 );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = 0;
...
ASEEND*/
//CHKSM=F8C35F02348F0B95417CB8C029D15813D6A4A7D3