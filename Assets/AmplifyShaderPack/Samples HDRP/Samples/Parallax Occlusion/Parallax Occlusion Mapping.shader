	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 OffsetPOM8 = POM( _HeightMap, uv_Albedo, ddx( uv_Albedo ), ddy( uv_Albedo ), NormalWS, V, ase_viewDirTS, 128, 128, 10, _Scale, 0, _HeightMap_ST.xy, appendResult47, 0 );
				...
				surfaceDescription.BaseColor = tex2D( _Albedo, customUVs39, temp_output_40_0, temp_output_41_0 ).rgb;
				surfaceDescription.Normal = unpack14;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = tex2D( _Metallic, customUVs39, temp_output_40_0, temp_output_41_0 ).r;

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = 0;
				#endif

				surfaceDescription.Smoothness = ( 1.0 - ( _RoughScale * tex2D( _Roughness, customUVs39, temp_output_40_0, temp_output_41_0 ).r ) );
				surfaceDescription.Occlusion = tex2D( _Occlusion, customUVs39, temp_output_40_0, temp_output_41_0 ).r;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 appendResult47 = (float2(_CurvatureU , _CurvatureV));
				float2 OffsetPOM8 = POM( _HeightMap, uv_Albedo, ddx( uv_Albedo ), ddy( uv_Albedo ), ase_normalWS, ase_viewDirWS, ase_viewDirTS, 128, 128, 10, _Scale, 0, _HeightMap_ST.xy, appendResult47, 0 );
				float2 customUVs39 = OffsetPOM8;
				float2 temp_output_40_0 = ddx( uv_Albedo );
				float2 temp_output_41_0 = ddy( uv_Albedo );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float3 ase_viewDirTS = normalize( ase_viewVectorTS );
				float2 appendResult47 = (float2(_CurvatureU , _CurvatureV));
				float2 OffsetPOM8 = POM( _HeightMap, uv_Albedo, ddx( uv_Albedo ), ddy( uv_Albedo ), ase_normalWS, V, ase_viewDirTS, 128, 128, 10, _Scale, 0, _HeightMap_ST.xy, appendResult47, 0 );
				float2 customUVs39 = OffsetPOM8;
				float2 temp_output_40_0 = ddx( uv_Albedo );
				float2 temp_output_41_0 = ddy( uv_Albedo );
				float3 unpack14 = UnpackNormalScale( tex2D( _Normal, customUVs39, temp_output_40_0, temp_output_41_0 ), _NormalScale );
...
WireConnection;48;4;23;1
WireConnection;48;7;42;0
WireConnection;48;8;21;1
ASEEND*/
//CHKSM=C897DAFE826E0EFA301D1F10DB56919587E2BF61